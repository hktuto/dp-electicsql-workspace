<script setup lang="ts">
  ...
  async function loadCompany() {
    ...
    try {
      ...
      const foundCompany = await companySync.findBySlug(props.slug)
      if (foundCompany) {
  ...
  async function loadMembers() {
    try {
      const response = await $fetch<{ members: Member[] }>(`/api/companies/${props.slug}/members`)
  ...
  async function saveSettings() {
    ...
    saving.value = true
    try {
      await $fetch(`/api/companies/${props.slug}`, {
        method: 'PUT',
        body: {
          name: form.name,
  ...
  async function sendInvite() {
    ...
    try {
      const response = await $fetch<{ invite: { inviteLink: string } }>(`/api/companies/${props.slug}/invites`, {
        method: 'POST',
        body: {
  ...
  async function cancelInvite(inviteId: string) {
    try {
      await $fetch(`/api/companies/${props.slug}/invites/${inviteId}`, {
        method: 'DELETE',
      })
  ...
  async function removeMember(memberId: string) {
    try {
      ...
      await $fetch(`/api/companies/${props.slug}/members/${memberId}`, {
        method: 'DELETE',
      })
