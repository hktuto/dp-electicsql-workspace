...
import type { ComponentNode } from '~/utils/type/apps'
const props = defineProps<{
    component: ComponentNode
}>()
const {displayMode} = useAppsContext()
        ...
        <NuxtErrorBoundary>
            <component ref="componentRef" :is="componentToRender" v-bind="cProps" v-on="evs">
                <template v-for="(children, key) in component.slots" :key="key" v-slot:[key]>
                    <template v-if="typeof children === 'string'">
                        {{ children }}
                    </template>
                    <template v-else>
                        <AppsComponent v-for="childComponent in children" :key="childComponent.instanceId" :component="childComponent" />
                    </template>
                </template>
            </component>
